"""
Created on Fri Jun  5 12:51:13 2020
@author: Thanh Le
Gabor layer version 4 with vectorization
"""
...
class Gabor2D(Layer):
    ...
    def call(self, inputs):
        ...
        x_kernels = np.transpose(x_kernels, (2, 3, 0, 1))  # this step is quite clumsy, can be improved later
        ...
        outputs = K.conv2d(inputs, self.gabor_kernels, strides=self.strides,
    ...
    def compute_output_shape(self, input_shape):
        ...
        if self.data_format == 'channels_first':  # input: [batch_size, #channels, rows, cols]
            space = input_shape[2:]
            new_space = []
            for i in range(len(space)):
                new_dim = conv_utils.conv_output_length(
